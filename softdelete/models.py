...
class SoftDeleteQuerySet(query.QuerySet):
    ...
    def restore(self, using='default', *args, **kwargs):
        logging.debug("RESTORING %s", self)
        for obj in self:
            cs = _determine_change_set(obj)
            cs.restore(using)
        logging.debug("FINISHED RESTORING %s", self)
...
class SoftDeleteManager(models.Manager):
    ...
    def get(self, *args, **kwargs):
        if 'pk' in kwargs:
            return self.all_with_trashed().get(*args, **kwargs)
        else:
            return self._get_self_queryset().get(*args, **kwargs)

    def filter(self, *args, **kwargs):
        if 'pk' in kwargs:
            qs = self.all_with_trashed().filter(*args, **kwargs)
        else:
...
class SoftDeleteObject(models.Model):
    ...
    class Meta:
        abstract = True
        permissions = (
            ('can_restore', 'Can restore this object'),
        )
    ...
    def force_delete(self, *args, **kwargs):
        logging.debug("FORCE DELETEING type %s, %s", type(self), self)
        try:
    ...
    def _do_restore(self, using='default'):
        pre_restore.send(sender=self.__class__,
                         instance=self,
                         using=using)
        self.deleted_at = None
        self.save()
        post_restore.send(sender=self.__class__,
                          instance=self,
                          using=using)
    ...
    def restore(self, using='default', *args, **kwargs):
        ...
        cs.restore(using)
        logging.debug('FINISHED RESTORING RELATED %s', self)
...
class ChangeSet(models.Model):
    ...
    def restore(self, using='default'):
        logging.debug("CHANGESET RESTORE: %s" % self)
        self.content._do_restore(using)
        for related in self.soft_delete_records.all():
            related.restore(using)
        self.delete()
        logging.debug("FINISHED CHANGESET RESTORE: %s", self)
...
def assign_permissions(user_or_group):
    for model in ['ChangeSet', 'SoftDeleteRecord']:
        ...
        p, pc = Permission.objects.get_or_create(
            name="Can restore a soft-deleted object",
            codename="can_restore",
            content_type=ct)
        permissions = [p]
